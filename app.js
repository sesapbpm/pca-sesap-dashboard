...
function renderMonths(){
  ...
  el.innerHTML=`<svg viewBox="0 0 ${w} ${h}">${months.map((n,i)=>{const x=p.l+i*gap+(gap-bw)/2;let base=h-p.b;const bars=['late','ontime','done'].map(status=>{const bh=series[status][i]/max*(h-p.t-p.b);base-=bh;return `<rect class="clickable schedule-bar" x="${x}" y="${base}" width="${bw}" height="${bh}" fill="${colors[status]}" opacity="${state.month&&state.month!==i+1?'.38':'1'}" data-month="${i+1}" data-tip="${n} · ${labels[status]}|${money(series[status][i])}"/>`}).join('');return `${bars}<text class="axis-label" x="${x+bw/2}" y="${h-9}" text-anchor="middle">${n.slice(0,3)}</text>`}).join('')}</svg>`;bindTips(el);el.querySelectorAll('[data-month]').forEach(n=>n.onclick=()=>{state.month=state.month===+n.dataset.month?null:+n.dataset.month;state.page=1;applyFilters()});
}
