...
function renderMonths(){
  const series={done:Array(12).fill(0),ontime:Array(12).fill(0),late:Array(12).fill(0)},linkMap=new Map((state.cycle?.vinculos||[]).map(v=>[v.chaveItemPca,v])),today=new Date().toISOString().slice(0,10);
  state.filtered.forEach(x=>{if(!x.dataDesejada)return;const m=+x.dataDesejada.slice(5,7)-1;if(m<0||m>11)return;const key=`${x.cnpj}|${x.anoPca}|${x.codigoUnidade}|${x.numeroItem}`,link=linkMap.get(key),status=link&&(link.idCompra||link.situacaoCiclo==='Iniciada (confirmada)'||link.situacaoCiclo==='Contrato formalizado')?'done':x.dataDesejada.slice(0,10)>=today?'ontime':'late';series[status][m]+=+x.valorTotal||0});
  const totals=Array.from({length:12},(_,i)=>series.done[i]+series.ontime[i]+series.late[i]),el=$('monthChart'),w=600,h=210,p={l:45,r:10,t:12,b:30},max=Math.max(...totals,1),bw=(w-p.l-p.r)/12*.58,gap=(w-p.l-p.r)/12,colors={done:'#16a05d',ontime:'#3b82f6',late:'#dc4c4c'},labels={done:'Iniciado/concluído',ontime:'No prazo',late:'Atrasado'};
  el.innerHTML=`<svg viewBox="0 0 ${w} ${h}">${months.map((n,i)=>{const x=p.l+i*gap+(gap-bw)/2;let base=h-p.b;const bars=['late','ontime','done'].map(status=>{const rawHeight=series[status][i]/max*(h-p.t-p.b),bh=series[status][i]>0?Math.max(rawHeight,4):0;base-=bh;return `<rect class="clickable schedule-bar" x="${x}" y="${base}" width="${bw}" height="${bh}" fill="${colors[status]}" opacity="${state.month&&state.month!==i+1?'.38':'1'}" data-month="${i+1}" data-tip="${n} · ${labels[status]}|${money(series[status][i])}"/>`}).join('');const marker=series.done[i]>0?`<circle class="clickable" cx="${x+bw/2}" cy="${Math.max(p.t,base-5)}" r="3.5" fill="${colors.done}" data-month="${i+1}" data-tip="${n} · ${labels.done}|${money(series.done[i])}"/>`:'';return `${bars}${marker}<text class="axis-label" x="${x+bw/2}" y="${h-9}" text-anchor="middle">${n.slice(0,3)}</text>`}).join('')}</svg>`;bindTips(el);el.querySelectorAll('[data-month]').forEach(n=>n.onclick=()=>{state.month=state.month===+n.dataset.month?null:+n.dataset.month;state.page=1;applyFilters()});
}
